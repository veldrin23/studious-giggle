...
1284101485
1636329600001 
def extract_updload_funding_rate(client):
    ...
    print(timestamp)
    print(datetime.utcfromtimestamp(int(timestamp/1000)).strftime('%Y-%m-%d %H:%M:%S'))
...
if __name__ == "__main__":
    conn = sqlite3.connect("./data/funding_rates.db")
    cur = conn.cursor()
    with open("./config/binance_secret_key.txt") as _:
        api_secret = _.read().splitlines()[0]
    with open("./config/binance_api_key.txt") as _:
    ...
    if len(cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='FUNDING_RATES'").fetchall()) == 0: 
        cur.execute("CREATE TABLE FUNDING_RATES (timestamp int, symbol TEXT, funding_rate float)")
    extract_updload_funding_rate(binance_client)
